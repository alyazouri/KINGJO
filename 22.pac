// ======================================================================
//  PUBG-JO-PURE-ONLY PAC — FINAL HARD MODE
//  - MATCH / RECRUIT / LOBBY = فقط JO_V4 / JO_V6
//  - أي سيرفر مش أردني → BLOCK + إعادة بحث
//  - CDN / UPDATES مسموح عادي عشان لا تخرب التحديثات
// ======================================================================
...
var FORCE_PUBG_THROUGH_PROXY = true;
var JO_PURE_ONLY_MODE       = true;   // ✅ أقسى وضع أردني
var JO_MATCH_STRICT_LEVEL   = 999;    // عدد اللانهائي تقريباً
var JO_MATCH_ATTEMPTS       = 0;
...
function ipv4ToLong(ip){
  var p=ip.split('.'),n=0;
  for(var i=0;i<4;i++) n=(n<<8)+parseInt(p[i]);
  return n>>>0;
}
function parseCIDR(c){
  ...
  b=ipv4ToLong(a[0]),
  m=~((1<<(32-a[1]))-1)>>>0;
  return {net:(b&m)>>>0,mask:m};
}
function isIpInCidr(ip,c){
  var o=parseCIDR(c),i=ipv4ToLong(ip);
  return (i&o.mask)>>>0===o.net;
}
...
function ipv6ToBigInt(ip){
  var p=expandIPv6(ip),v=0n;
  for(var i=0;i<8;i++) v=(v<<16n)+BigInt(p[i]);
  return v;
}
function isIp6InRange(ip,s,e){
  var v=ipv6ToBigInt(ip),a=ipv6ToBigInt(s),b=ipv6ToBigInt(e);
  return v>=a&&v<=b;
}
...
function isJOIP(ip){
  if(!ip) return false;
  return ip.indexOf(":")!==-1?isJOIPv6(ip):isJOIPv4(ip);
}
...
function FindProxyForURL(url, host){
  ...
  if(isPUBGHost(host)){
    ...
    // ✅ PURE ONLY MODE
    if(JO_PURE_ONLY_MODE){
      if(role==="MATCH"||role==="RECRUIT_SEARCH"||role==="LOBBY"){
        if(!isJO){
    ...
    return "PROXY "+PROXY_IP+":"+FIXED_PORT[role]+";";
  }
