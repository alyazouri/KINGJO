// ===================================================
//  PUBG Mobile – JO EXTREME++ ULTIMATE (THE ABSOLUTE BEST)
//  - IP-based Jordan enforcement (latest IPs, IPv4+IPv6 support)
//  - MATCH 20001 ONLY (with adaptive pressure & smart chaining)
//  - No DIRECT (except GitHub + YouTube + more)
//  - Ultimate: Updated data, performance, IPv6, logging, smart proxy selection
// ===================================================

// ===== CONFIGURABLE OPTIONS (Ultimate Customization) =====
var ENABLE_PRESSURE = true;          // تمكين الضغط الأقصى
var ADAPTIVE_PRESSURE = true;        // ضغط تكيفي بناءً على الفشل
var PRESSURE_LEVEL = 17;             // مستوى الضغط الأساسي (0-20)
var ENABLE_LOGGING = false;          // تمكين التسجيل للاختبار
var MAX_CHAIN_LENGTH = 3;            // طول السلسلة القصوى (1-5)
var ENABLE_IPV6 = true;              // دعم IPv6
var FALLBACK_MODE = true;            // وضع الاحتياطي إذا فشلت البروكسيات

// ===== DIRECT ALLOW (Ultimate: More safe domains) =====
var DIRECT_ALLOW = [
  ".github.com", ".githubusercontent.com", ".raw.githubusercontent.com",
  ".youtube.com", ".googlevideo.com", ".ytimg.com", "youtu.be",
  ".google.com", ".gstatic.com", ".apple.com", ".microsoft.com"  // إضافات للأمان
];

// ===== SOFTBLOCK =====
var BLOCK = "PROXY 127.0.0.1:9";

// ===== PORTS (Ultimate: More for updates, voice, and new features) =====
var PORT_MATCH = [20001];
var PORT_LOBBY = [443, 8080, 17500, 18081, 8443, 993];  // إضافة 993 للأمان
var PORT_VOICE = [3478, 3479, 5000, 5001, 5002, 19302, 19303];  // دعم Agora الجديد
var PORT_UPD = [80, 7080, 443, 993];  // إضافة 993

// ===== PUBG DOMAINS (Ultimate: Latest Tencent/Agora domains) =====
var PUBG_DOMAINS = [
  ...
  ".tencentcloud.com", ".qcloud.com", ".webrtc.agora.io"  // إضافات لـ WebRTC
];

// ===== JO IP RANGES (Ultimate: Latest IPv4 + IPv6 from RIPE NCC) =====
var JO_IP_RANGES_V4 = [
  ...
  "185.76.12.0/22", "185.76.16.0/22", "185.76.20.0/22"  // تحديثات 2024
];
var JO_IP_RANGES_V6 = [
  "2a00:1f48::/32", "2a00:1f49::/32"  // نطاقات IPv6 الأردنية الرئيسية
];

// ===== MATCH POOL (Ultimate: More proxies with low-latency selection) =====
var MATCH_POOL = [
  ...
  "PROXY 176.29.198.75:20001", "PROXY 188.247.95.100:20001",  // إضافات
  "PROXY 212.118.5.20:20001", "PROXY 213.139.58.30:20001"
];

// ===== LOBBY POOL (Ultimate: More proxies) =====
var LOBBY_POOL = [
  ...
  "PROXY 188.247.95.50:443", "PROXY 176.29.211.200:443",  // إضافات
...
// ===== CACHING & STATE =====
var dnsCache = new Map();  // Cache لـ DNS للأداء
var failureCount = 0;      // عداد الفشل للضغط التكيفي

// ===== HELPERS (Ultimate: Faster with Maps, IPv6 support, validation) =====
...
function inCIDR6(ip, cidr) {  // دعم IPv6
  // تبسيط: تحقق من البادئة (يمكن توسيعها)
...
function validateProxy(proxy) {  // فحص بسيط للصحة
  // في PAC، لا يمكن فحص حقيقي، لكن يمكن إضافة منطق افتراضي
  return proxy.includes("PROXY");  // افتراضي: صحيح إذا كان PROXY
}
...
function pick(pool, seed) {
  ...
  if (!validateProxy(selected)) return pick(pool, seed + "retry");  // إعادة المحاولة
  return selected;
}
...
// ===== CORE (Ultimate: Adaptive pressure, smart chaining, IPv6) =====
function FindProxyForURL(url, host) {
  ...
  if (isDomainIn(host, pubgDomainsMap)) {
    ...
    // ===== EXTREME++ GATE (Ultimate: Adaptive pressure) =====
    if (!isJO && port === 20001 && ENABLE_PRESSURE) {
      let currentPressure = PRESSURE_LEVEL;
      if (ADAPTIVE_PRESSURE) {
        currentPressure = Math.min(20, PRESSURE_LEVEL + Math.floor(failureCount / 10));  // زيادة مع الفشل
      }
      ...
      failureCount = Math.max(0, failureCount - 1);  // نجاح: تقليل العداد
    }
